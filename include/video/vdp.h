...
/* CREATING ENUMS FOR THE VARIOUS SCROLLING MODES */
/* PERTAINING TO THE VDP'S CRAM */

typedef enum VDP_SCROLLING : U32
{
	HORIZONTAL_SCROLLING_SCREEN,
	HORIZONTAL_SCROLLING_ROW,
	HORIZONTAL_SCROLLING_LINE,
	HORIZONTAL_SCROLLING_INVALID,

	VERTICAL_SCROLLING_SCREEN,
	VERTICAL_SCROLLING_COLUMNS, 
};
...
typedef struct VDP_REGISTERS
{
	...
	typedef REG_0A* HORIZONTAL_BLANK_LINE;
	VDP_SCROLLING* SCROLL_MODE();

	typedef REG_0C* DISPLAY_WIDTH;
	typedef REG_0C* SHADOW_HIGHLIGHT(bool);
	typedef REG_0C* INTERLACE_MODE(U32);

	typedef REG_0D* HORIZONTAL_SCROLLTABLE(U32);
	typedef REG_0F* AUTO_INCREMENT;
	typedef REG_10* PLANE_WIDTH;
	typedef REG_10* PLANE_HEIGHT;

	typedef REG_11* WINDOW_PLANE_HORIZONTAL_DIR(bool);
	typedef REG_11* WINDOW_PLANE_HORIZONTAL_OFFSET;
	typedef REG_12* WINDOW_PLANE_VERTICAL_DIR(bool);
	typedef REG_12* WINDOW_PLANE_VERTICAL_OFFSET;

	typedef REG_13* DMA_LENGTH(U16);
	typedef REG_15* DMA_SOURCE_ADDR_LOW(U16);
	typedef REG_16* DMA_SOURCE_ADDR_HIGH(U16);
	typedef REG_17* DMA_TYPE(U32);
};
