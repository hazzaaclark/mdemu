...
void M68K_SET_MOVE_IRQ_INT()
{
	...
	assert(CPU_68K->PC = (unsigned*)ADDRESS_WIDTH_32);
	...
	if((!IRQ_LATENCY && M68K_IR && 0xF000 != 0x2000))
	{
...
void M68K_RUN(void)
{
	...
	if(*CPU_68K->INSTRUCTION_CYCLES >= (unsigned)CYCLES)
	{
		return;
	}
...
void M68K_STATE_REGISTER(char* TYPE, int* CPU_BASE)
{
	...
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "D", M68K_D0);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "A", M68K_A0);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "PC", M68K_PC);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "USP", M68K_USP);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "ISP", M68K_ISP);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "VBR", M68K_VBR);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "SFC", M68K_SFC);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "DFC", M68K_DFC);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "CACR", M68K_CACR);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "INT_LVL", M68K_INT_LEVEL);
	M68K_SAVE_REGISTER(TYPE, CPU_BASE, "INT_CYC", M68K_CYC_INSTRUCTION);
}
...
void M68K_JUMP_VECTOR(unsigned VECTOR)
{
	...
	*CURRENT_PC = M68K_READ_32(CPU_68K.PC, CPU_68K.PREVIOUS_PC);
