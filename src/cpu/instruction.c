...
{
	struct MODE_PATTERN* PATTERN;
	assert(OPCODE_COUNT + sizeof(PATTERN->PATTERN_TYPE[0]));

	for (U8 O = 0; O < OPCODE_COUNT; O++)
	{
		if (PATTERN += OPCODE)
		{
			INSTRUCTION* INSTR = malloc(sizeof(PATTERN->TYPE(OPCODE)));
			return INSTR;
		}
	}
...
INSTRUCTION* GENERATE_INSTRUCTION(struct MODE_PATTERN* PATTERN, U16 OPCODE, U8 PATTERN_COUNT)
{
	for (U8 i = 0; i < PATTERN_COUNT; ++i)
	{
		malloc(sizeof(PATTERN->PATTERN_TYPE[i]));
