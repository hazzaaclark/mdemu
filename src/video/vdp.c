...
void VDP_CONST_INIT(VDP* VDP, VDP_STATE* STATE		)
{
	VDP = malloc(sizeof(struct VDP));
...
unsigned int VDP_READ_BYTE(unsigned int ADDRESS)
{
	...
	switch(ADDRESS & 0xFD)
	{
		...
		case 0x04:
			...
			ADDRESS = M68K_REG_PC;
			DATA |= (M68K_READ_BUS_BYTE((CPU_BASE->MEMORY_MAP[((ADDRESS) >> 16) & 0xFF].MEMORY_BASE)));
			return DATA;
